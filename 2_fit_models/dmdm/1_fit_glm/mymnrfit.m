function [b,dev,stats] = mymnrfit(x,y,varargin)
...
%   
%      'Tolerance' - a positive scalar, which determines how close the
%                    estimated parameters must be between iterations to 
%                    consider them as having converged. The default is 1e-6. 
%
%      'IterationLimit' - A positive integer, which determines the maximum
%                         number of iterations the algorithm runs for. The 
%                         default is 100.
%
%       'Weights' - A double/single vector of non-negative observations
%                   weights. The vector must have the same number of rows as
%                   the number of observations. The default is a vector of
%                   1s.
%
...
pnames = { 'Model' 'Interactions' 'Link' 'EstDisp' 'Tolerance', 'IterationLimit', 'Weights'};
dflts =  {'nominal'             []     []     'off', 1e-6, 100, []};
[model,interactions,link,estdisp,tolfit,iterlim, weights] = ...
...
validateattributes(tolfit, {'double', 'single'}, {'scalar', 'real', 'positive', 'nonsparse'}, ...
    mfilename, 'Tolerance');
validateattributes(iterlim, {'double', 'single'}, {'real', 'positive', 'integer', 'nonsparse'}, ...
    mfilename, 'IterationLimit');

% Non-numeric responses 
y = MultinomialRegression.validateY(y, false, size(x, 1));
if islogical(y)
    % Convert to numeric directly. Y can also be a logical matrix of counts
    y = double(y);
    if isvector(y)
        % Can't have class names of 0
        y = y + 1;
    end
end
if ~isfloat(y)
...
if isempty(weights)
    weights = ones(size(y, 1), 1, dataClass);
else
    validateattributes(weights, {'double', 'single'}, {'vector', 'nonnegative', 'real', 'nonsparse', 'numel', size(y,1)}, ...
        mfilename, 'Weights');
end

% Remove missing values from the data.  Also turns row vectors into columns.
[anybad,wasnan,y,x,weights] = statremovenan(y,x,weights);
if anybad
...
if k == 1
    ...
    y = accumarray({(1:n)' y},ones(dataClass)) .* weights;
    k = size(y,2);
    m = weights;
else
    y = y .* weights;
...
if strcmp(model,'hierarchical')
    if nargout < 3
        [b,dev] = hierarchicalFit(x,y,m,link,n,k,p,pstar,parallel,estdisp,iterlim,tolfit);
    else
        [b,dev,stats] = ...
            hierarchicalFit(x,y,m,link,n,k,p,pstar,parallel,estdisp,iterlim,tolfit);
    end
else
    ...
    if strcmp(model,'nominal')
        [b,hess,pi] = nominalFit(x,y,m,pi,n,k,p,pstar,parallel,iterlim,tolpos,tolfit);
    else % 'ordinal'
        z = cumsum(y(:,1:(k-1)),2);
        [b,hess,pi,gam] = ...
            ordinalFit(x,z,m,pi,flink,ilink,dlink,n,k,p,pstar,parallel,iterlim,tolpos,tolfit);
    end
...
%------------------------------------------------------------------------
function [b,XWX,pi,gam] = ordinalFit(x,z,m,pi,flink,ilink,dlink,n,k,p,pstar,parallel,iterLim,tolpos,convcrit)
...
%------------------------------------------------------------------------
function [b,XWX,pi] = nominalFit(x,y,m,pi,n,k,p,pstar,parallel,iterLim,tolpos,convcrit)
...
while iter <= iterLim
    ...
    eta = [eta zeros(n,1,'like',eta)]; %#ok<AGROW> 
...
%------------------------------------------------------------------------
function [b,dev,stats] = hierarchicalFit(x,y,m,link,n,k,~,pstar,parallel,estdisp,iterlim,tolfit)
    ...
    if parallel
        ...
        if nargout < 3
            [b,dev] = glmfit(xstar,[ystar(:) m(:)],'binomial',...
                'link',link,'constant','off','estdisp',estdisp,'options',opts);
        ...
        if nargout < 3
            for j = 1:k-1
                [b(:,j),d] = glmfit(x,[y(:,j) m(:,j)], 'binomial','link',link,'options',opts);
                needToWarn = checkForIterWarn(needToWarn);
                dev = dev + d;
            end
        else
            ...
            for j = 1:k-1
                [b(:,j),d,s] = glmfit(x,[y(:,j) m(:,j)], 'binomial','link',link,'options',opts);
                needToWarn = checkForIterWarn(needToWarn);
                dev = dev + d;
                stats.beta(:,j) = b(:,j);
...
if nargout > 2
    stats.covb = stats.coeffcorr .* (stats.se(:) .* (stats.se(:)'));
end
[~,wid] = lastwarn;
if needToWarn
    warning(message('stats:mnrfit:IterOrEvalLimit'));
